...
如果还没有发布到 npm registry，或者你想从源码测试本地版本，再用源码安装：
...
```text
Codex 对话
  -> Micat hooks 记录 user message + assistant final answer
  -> 每 N 轮滚动压缩
  -> 生成 micat_context.md 和 active_rules.md/json
  -> 在 UserPromptSubmit / SessionStart(compact) 时注入规则
```
